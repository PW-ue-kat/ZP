...
for i, c in enumerate(cnts):
    ...
    if (minimal_px_size < area  < maximum_px_size):
    ...
    # then draw the contours and the name of the shape on the image
        ...
        resized_area = cv2.contourArea(c)
        puzzle_counter += 1
        ...
        cv2.putText(image, f"{puzzle_counter}", (mid_x, mid_y), cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 255, 0),
                    thickness=2)
        kostki.append({'id': i, 'width': w, 'height': h, 'area': resized_area})

df = pd.DataFrame(kostki, columns=['id', 'width', 'height', 'area'])

print(f"Liczba wykrytych kostek: {len(df)}" )
print(f"średnia szerokość: {df['width'].mean()}" )
print(f"średnia wysokość: {df['height'].mean()}" )
print(f"Minimalny rozmiar: {df['area'].min()}" )
print(f"Maksymalny rozmiar: {df['area'].max()}" )
